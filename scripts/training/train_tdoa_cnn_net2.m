...
layers = [
    ...
    % --- Block 1 ---
    convolution2dLayer([20 1], 8,  'Padding','same', 'Name','conv1')
    batchNormalizationLayer('Name','bn1')
    reluLayer('Name','relu1')
    maxPooling2dLayer([2 1], 'Stride',[2 1], 'Name','pool1')   % 110 -> 55

    % --- Block 2 (Option B pooling) ---
    convolution2dLayer([10 1], 16, 'Padding','same', 'Name','conv2')
    batchNormalizationLayer('Name','bn2')
    reluLayer('Name','relu2')
    maxPooling2dLayer([2 1], 'Stride',[2 1], 'Name','pool2')   % 55 -> 28

    % --- Block 3 ---
    convolution2dLayer([5 1], 32, 'Padding','same', 'Name','conv3')
    batchNormalizationLayer('Name','bn3')
    reluLayer('Name','relu3')
...
%% === Save Results ===
save(fullfile(projectRoot, 'models', 'active', 'trained_tdoa_net_cnn_2.mat'), ...
