...
%
% Load the complete configuration with:
%   config = load_experiment_config("baseline");
%
% Nested fields are preferred. For compatibility, the former flat fields
% sourceDir, outputDir, writeOutput, and overwrite remain accepted.
    ...
    specs = config.evaluation.artifacts;

    perFlight = table();
    referenceLabels = strings(0, 1);
    for i = 1:size(specs, 1)
        artifactPath = fullfile(config.paths.sourceDir, specs{i, 1});
        ...
        [artifactRows, labels] = readMetricArtifact( ...
            artifactPath, specs{i, 2}, specs{i, 3}, config);
    ...
    expectedFlightCount = config.dataset.expectedFlightCount;
    ...
    metadata.sourceDir = config.paths.sourceDir;
    metadata.configurationName = config.name;
    ...
    if config.evaluation.writeOutput
...
function config = resolveConfig(requested)
    requested = mapLegacyFlatFields(requested);
    config = load_experiment_config("baseline", requested);

    config.paths.repositoryRoot = char(string(config.paths.repositoryRoot));
    config.paths.sourceDir = char(string(config.paths.sourceDir));
    config.paths.outputDir = char(string(config.paths.outputDir));
    validateattributes(config.dataset.expectedFlightCount, {'numeric'}, ...
        {'scalar', 'integer', 'positive'});
    validateattributes(config.models.names, {'string', 'cell'}, {'vector'});
    config.models.names = string(config.models.names(:));
    validateattributes(config.evaluation.writeOutput, {'logical', 'numeric'}, {'scalar'});
    validateattributes(config.evaluation.overwrite, {'logical', 'numeric'}, {'scalar'});
    config.evaluation.writeOutput = logical(config.evaluation.writeOutput);
    config.evaluation.overwrite = logical(config.evaluation.overwrite);
end

function requested = mapLegacyFlatFields(requested)
    mappings = {
        'sourceDir',  'paths',      'sourceDir';
        'outputDir',  'paths',      'outputDir';
        'writeOutput','evaluation', 'writeOutput';
        'overwrite',  'evaluation', 'overwrite'
    };
    for i = 1:size(mappings, 1)
        oldName = mappings{i, 1};
        group = mappings{i, 2};
        newName = mappings{i, 3};
        if isfield(requested, oldName)
            if ~isfield(requested, group) || ~isstruct(requested.(group))
                requested.(group) = struct();
            end
            requested.(group).(newName) = requested.(oldName);
            requested = rmfield(requested, oldName);
        end
    end
end

function [rows, labels] = readMetricArtifact(artifactPath, task, metric, config)
    ...
    rows = table( ...
        repelem(string(config.dataset.constellation) + "-" + labels, nModels), ...
        repelem(repmat(string(config.dataset.constellation), nFlights, 1), nModels), ...
...
function writeResults(results, config)
    if ~isfolder(config.paths.outputDir)
        mkdir(config.paths.outputDir);
    end

    outputFiles = cellfun(@(name) fullfile(config.paths.outputDir, name), ...
        config.evaluation.outputFiles, 'UniformOutput', false);
    if ~config.evaluation.overwrite
