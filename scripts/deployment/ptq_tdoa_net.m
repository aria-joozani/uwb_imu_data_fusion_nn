...
inFile = fullfile(projectRoot, 'models', 'active', 'trained_tdoa_net_cnn_2.mat');
...
intermediateDir = fullfile(projectRoot, 'local-artifacts', 'intermediates');
load(fullfile(intermediateDir, 'prepared_data_for_ptq.mat'), ...
    'XCal4D', 'XTest4D', 'YTest');
...
if ~isfolder(intermediateDir)
    mkdir(intermediateDir);
end
outFile = fullfile(intermediateDir, 'trained_tdoa_net_cnn_2_INT8_PTQ.mat');
